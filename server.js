...
app.get("/api/status", (req,res)=>res.json(payload()));

app.get("/api/qr", async (req,res)=>{
  const host = req.query.host || getIPs()[0] || "localhost";
  const url = `http://${host}:${PORT}`;
  try {
    const data = await QRCode.toDataURL(url, { margin: 1, width: 420 });
...
io.on("connection", socket => {
  ...
  socket.on("answer", data => {
    ...
    if (!["phishing","ai"].includes(q) || !["A","B","C"].includes(choice)) return;

    const old = socket.data.answers[q];
    if (old && old !== choice) state[q][old] = Math.max(0, state[q][old] - 1);
    if (!old || old !== choice) state[q][choice]++;
  ...
  socket.on("reset", () => {
    if (socket.handshake.auth?.admin === "forum2026") {
      ...
      state.phishing = { A: 0, B: 0, C: 0 };
      state.ai = { A: 0, B: 0, C: 0 };
      state.completed = 0;
      io.emit("state", payload());
    }
  });
...
server.listen(PORT, "0.0.0.0", ()=>{
  console.log(`Somali Media CyberSafe Live running on http://localhost:${PORT}`);
  console.log("On the same Wi-Fi, use one of these addresses:");
  for (const ip of getIPs()) console.log(`  http://${ip}:${PORT}`);
});